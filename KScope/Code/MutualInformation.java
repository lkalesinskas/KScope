...
/**
 * Implements common discrete Mutual Information functions. Provides: Mutual
 * Information I(X;Y), Conditional Mutual Information I(X,Y|Z). Defaults to
...
public abstract class MutualInformation {
  ...
  public strictfp static double calculateMutualInformation(double[] firstVector, double[] secondVector)
  {
    ...
    try{
	    ...
	    {
	    	key = Math.abs(key);
//	    	System.out.println("key being used " + key +"\nand it is getting " + state.jointProbMap.get(key));
	    	if(state.jointProbMap.get(key) == null) continue;
	 ...
	 *
	 * @param firstVector
	 *            Input vector (X). It is discretised to the floor of each value
	 *            before calculation.
	 * @param secondVector
	 *            Input vector (Y). It is discretised to the floor of each value
	 *            before calculation.
	 * @param conditionVector
	 *            Input vector (Z). It is discretised to the floor of each value
	 *            before calculation.
	 * @return The conditional Mutual Information I(X;Y|Z).
	 */
	public static double calculateConditionalMutualInformation(double[] firstVector, double[] secondVector,
